...
public class LoginController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 인증처리 : id,pw 값을 service로 전달하고 -> dao 전달해서 MmeberDTO객체를 반환
		MemberDTO member =loginService.loginService(mid, pw);
		// 		   MemberDTO => null 이면 회원이 아니다. 
		// 						null 이 아니면 회원이다 -> 로그인처리
		if (member != null) {
			// 회원이다!!!! -> HttpSession 객체에 회원정보를 저장
			// 저장된 회원정보는 회원이 로그인 했다 의 판단기준으로 사용
